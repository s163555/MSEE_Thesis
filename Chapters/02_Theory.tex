...
\begin{figure}[ht]
	\centering
	\includegraphics[width=.6\textwidth]{2_plane_wave_jensen-cropped.pdf}
...
\begin{table}[ht]
	...
	\begin{tabularx}{\linewidth}{@{}lSSS@{}}
		\toprule
		\textbf{Medium}          & {\makecell{\textbf{Density ($\rho_{0}$)}\\\unit[per-mode = symbol]{\kilogram\per\meter\cubed}}} & {\makecell{\textbf{Speed of sound ($c$)}\\\unit[per-mode = symbol]{\meter\per\second}}} & {\makecell{\textbf{Characteristic}\\\textbf{acoustic impedance ($Z$)}\\\unit[per-mode = symbol]{\kilogram\per\meter\squared\per\second}}} \\ \midrule
		Air             & 1.2     & 333            & 0.4e3       \\
...
The backscattering coefficient, which characterizes scattering from a volume of scatterers, is another measure of scattering strength. It is defined as average received power per steradian volume of scatterers when flooded with plane waves of unit amplitude and the unit is \si{1\per\centi\meter\steradian}.
...
\begin{table}[ht]
	...
	\begin{tabularx}{\linewidth}{@{}lS@{}}
		\toprule
		\textbf{Tissue}          & {\makecell{\textbf{Attenuation}\\\si[per-mode = symbol,inter-unit-product =$\cdot$]{\dB\per\mega\hertz\per\centi\meter}}} \\ \midrule
		Liver & \numrange{0.6}{0.9}{} \\
		Kidney & \numrange{0.8}{1}{} \\
		Spleen & \numrange{0.5}{1}{}\\
		Fat & \numrange{1}{2}{} \\
		Blood & \numrange{0.17}{0.24}{} \\
		Plasma & 0.01 \\
		Bone & \numrange{16}{23}{} \\ \bottomrule
	\end{tabularx}
	\caption[Approximate attenuation values for human tissue]{Approximate attenuation values for human tissue \cite{JensenUltrasoundBook}}
	\label{tab:2_attenuation_tissue}
\end{table}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Capillaries, thin but wide ranging blood vessels that performs the exchange of matter between the blood and tissues.
...
The basic principles of the device is previously explained in the Doppler effect section, and the measurement of the device is described in \cref{eq:2_doppler_effect}. The device continuously emits an ultrasonic wave in the first transducer expressed as a function of time by \cref{eq:2_cw_tx} \cite{JensenUltrasoundBook}.
...
As is general for quadrature demodulation, the resulting signal contains the frequency components of the sum and difference of the emitted and received signals' frequencies. Generally a gls bp filter is used on the demodulated signal to remove the high frequency summed signal at twice the frequency of $f_{0}$. The filtered signal after the bp filter is expressed by \cref{eq:2_cw_bp} and contains the Doppler shift of the emitted signal.
...
\end{equation}
where the second exponential term is the delay proportional to the time between transmission and receiving of the signal.

\begin{table}[ht]
	\centering
	\begin{tabularx}{\linewidth}{@{}SS@{}}
		\toprule
		{\makecell{\textbf{Velocity ($v$)}\\\unit[per-mode = symbol]{\meter\per\second}}}          & {\makecell{\textbf{Doppler frequency ($f_{d}$)}\\\unit{\hertz}}} \\ \midrule
		0.01 & 28 \\
		0.1 & 276 \\
		0.5 & 1377\\
		1 & 2755 \\
