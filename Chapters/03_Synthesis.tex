...
\begin{align}
	d &= v \cdot t \Longrightarrow \\
	d &= \qty{1480}{\meter\per\second} \cdot \qty{15}{\micro\second} = \qty{11.55}{\milli\meter}
\end{align}
\end{subequations}
This means that a distance of less than \qty{11.55}{\milli\meter} to a scatterer is likely to produce a compromised measurement. The ultrasound switch is designed to switch the transmit and receive paths at specific times, as determined by the the input signals. A PCB design was implemented in Altium Designer \cite{altium} utilising three channels of the maximum eight available channels in the IC. Seen in \cref{fig:3_ultrasoundswitch} is a 3D render of the designed PCB.
...
\begin{figure}[htbp]
	...
	\resizebox{.6\textwidth}{!}{%
		\begin{circuitikz}
			\tikzstyle{every node}=[font=\LARGE]
			\draw (12.5,14.25) node[op amp,scale=1, yscale=-1 ] (opamp2) {};
			\draw (opamp2.+) to[short] (11,14.75);
			\draw  (opamp2.-) to[short] (11,13.75);
			\draw (13.7,14.25) to[short](14,14.25);
			\draw (10,14.75) to[C,l={ \normalsize $C_1$}] (8,14.75);
			\draw [](10,14.75) to[short] (11,14.75);
			\draw (8,12.75) to (8,12.5) node[ground]{};
			\draw (14,14.25) to[R,l={ \normalsize $R_2$}] (14,12.75);
			\draw[] (14,12.75) to[short] (11,12.75);
			\draw [](11,12.75) to[short] (11,13.75);
			\draw (14,12.75) to[R,l={ \normalsize $R_3$}] (14,11.25);
			\draw (14,11.25) to[C, l={\normalsize $C_2$}] (14,10.25);
			\draw (14,10.25) to (14,10) node[ground]{};
			\draw (8,14.75) to[sinusoidal voltage source, sources/symbol/rotate=auto,l_={ \normalsize $v_{\mathrm{in}}$}] (8,12.75);
			\draw (14,12.75) to[short, -*] (14,12.75);
			\draw [](14,14.25) to[short, -o] (15,14.25);
			\draw (14,14.25) to[short, -*] (14,14.25);
			\draw (11,14.75) to[R,l={ \normalsize $R_1$}] (11,16.25);
			\draw (11,14.75) to[short, -*] (11,14.75);
			\draw (10,17) to[american voltage source,l_={ \normalsize $V_{DC}$}] (10,15.75);
			\draw (10,15.75) to (10,15.5) node[ground]{};
			\draw [](10,17) to[short] (10,17.25);
			\draw [](10,17.25) to[short] (11,17.25);
			\draw [](11,17.25) to[short] (11,16.25);
		\end{circuitikz}
	}%
	\caption{Circuit diagram of \gls{hp} PRF and Wall filter}
	\label{fig:3_prf_wall_diagram}
\end{figure}
Finally, the signal should be DC coupled for sampling. In the circuit diagram in \cref{fig:3_prf_wall_diagram}, a \gls{hp} filter is used as a combined \gls{prf} and Wall-filter in conjunction with a \qty{2}{\decibel} gain amplification to maximize dynamic range of the \gls{adc} measurements from \qtyrange{0}{3.3}{\volt}. In the diagram, $V_{DC}=\qty{1.65}{\volt}$, which is half of the \gls{adc} dynamic range of \qtyrange{0}{3.3}{\volt}, and $v_{\mathrm{in}}$ is the input signal to the filter from the output of the sample-and-hold amplifier. A SPICE simulation was implemented and can be seen in \cref{fig:app_ltspice_dc_coupler}. From this simulation model, a small signal analysis as well as a transient analysis was conducted. The resulting small signal analysis can be seen in \cref{fig:3_dccoupler_sim_ac} and the transient analysis can be seen in \cref{fig:3_dccoupler_sim_transient}.
\begin{figure}[htbp]
	\centering
