...
A CMUT is made up of a cavity, membrane, and electrode, with two plate-like electrodes biased with a DC voltage and driven with an AC signal to move one of the plates harmonically. By using common integrated circuit fabrication processes, a capacitor cell appears as a metallized membrane suspended above a heavily doped silicon substrate, with an insulating layer to prevent shorting in case of contact. Many small capacitor cells are connected in parallel to create a single transducer element, and by organizing these elements in different geometries, any array shape is possible. (See Figure 1 and Figure 2 for visual representations.)
