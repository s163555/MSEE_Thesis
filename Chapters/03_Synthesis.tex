...
To build an application with the Zephyr kernel, you use CMake which has two phases - configuration and build. During configuration phase, CMake executes the CMakeLists.txt build scripts to generate an internal model of the Zephyr build. The configuration phase can be seen in \cref{fig:3_cmake_configuration}. Once configuration is done, CMake generates build scripts that are native to the host platform using Ninja or Make. Afterwards, the generated build scripts are executed to begin the second phase, build. The build scripts can recompile the application without involving CMake after most code changes. However, in some cases, you need to execute the configuration step again before building. Zephyr uses CMake’s 'target' concept to organize the build, where a target can be an executable, a library, or a generated file. The final output of the build system \texttt{ninja} is a binary file ready for \gls{flashing} using a microcontroller programmer.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=.8\textwidth]{Figures/3_cmake_configuration.svg}
	\caption[Configuration phase of a Zephyr application]{Configuration phase of a Zephyr application \cite{zephyrprojectdocumentation}, starting with \gls{dts}/\gls{dtsi} and then using the device tree and Kconfig to configure the set of build files for \texttt{ninja}}
	\label{fig:3_cmake_configuration}
\end{figure}
After cmake configuration phase has completed, the build phase begins. CMake invokes \texttt{ninja}, which (conceptually) has six (five) stages: pre-build, generation and compilation, first-pass binary (and second-pass binary), final binary and post-processing.
\begin{figure}[htbp]
	\centering
	\includegraphics[width=.8\textwidth]{Figures/3_cmake_build1.svg}
	\caption[Flowchart of ninja stage 1, pre-build]{Flowchart of \texttt{ninja} build stage 1, pre-build}
	\label{fig:3_build1}
\end{figure}
In \cref{fig:3_build1} the build system binds system call functions to implementations.
\begin{figure}[htbp]
	\centering
	\includegraphics[width=.8\textwidth]{Figures/3_cmake_build2.svg}
	\caption[Flowchart of ninja stage 2, generation and compilation]{Flowchart of \texttt{ninja} build stage 2, generation and compilation}
	\label{fig:3_build2}
\end{figure}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=.8\textwidth]{Figures/3_cmake_build3.svg}
	\caption[Flowchart of ninja stage 3, intermediate binary]{Flowchart of \texttt{ninja} build stage 3, intermediate binary}
	\label{fig:3_build3}
\end{figure}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=.8\textwidth]{Figures/3_cmake_build4.svg}
	\caption[Flowchart of ninja stage 4, second intermediate binary]{Flowchart of \texttt{ninja} build stage 4, second intermediate binary}
	\label{fig:3_build4}
\end{figure}
\Cref{fig:3_build4} shows stage 4, where the fixed size intermediate binary is generated. The previous stage's binaries are not fully formed and contain sections that are empty or marked as placeholders. These sections must be filled in by a process similar to reflection. To finish building, certain scripts are run on the intermediate binaries. These scripts generate the necessary components that are missing from the final binary.
\begin{figure}[htbp]
	\centering
	\includegraphics[width=.8\textwidth]{Figures/3_cmake_build5.svg}
	\caption[Flowchart of ninja stage 5, final binary]{Flowchart of \texttt{ninja} build stage 5, final binary}
	\label{fig:3_build5}
\end{figure}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=.8\textwidth]{Figures/3_cmake_build5.svg}
	\caption[Flowchart of ninja stage 6, final binary]{Flowchart of \texttt{ninja} build stage 6, post-processing}
	\label{fig:3_build6}
\end{figure}
