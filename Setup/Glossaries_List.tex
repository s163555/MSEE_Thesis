...
\newglossaryentry{open-source}{
	name={open-source},
	description={Refers to software whose original source code is publicly available and may be changed and distributed},
	category={glosscat}
}

\newglossaryentry{flashing}{
	name={flashing},
	description={Refers to the process of programming the non-volatile memory of a microcontroller},
	category={glosscat}
}

\newglossaryentry{monochromatic wave}{
	name={monochromatic wave},
	description={A wave which has only a single frequency},
	category={glosscat}
}

\newglossaryentry{haemodynamic}{
	name={haemodynamic},
	description={The dynamic movement of blood, or study of blood flow},
	category={glosscat}
}

\newglossaryentry{bitstream}{
	name={bitstream},
	description={A bitstream is a file that is created to setup an FPGA. The bitstream describes the hardware logic, routing, and starting values for registers as well as on-chip memory (e.g., LUT). A bistream, like the ELF format, has its own format for describing its contents},
	category={glosscat}
}

\newglossaryentry{soft microprocessor}{
	name={soft microprocessor},
	description={Also referred to as a softcore microprocessor, or soft processor, an entirely logical synthesis-based microprocessor core, usually deployed on an \gls{fpga}},
	category={glosscat}
}
...
\newabbreviation[category={acronymcat}]{mosfet}{MOSFET}{metal-oxide-semiconductor field-effect transistor}
...
\newabbreviation[category={acronymcat}]{dtsi}{DTSI}{device tree source include}

\newabbreviation[category={acronymcat}]{api}{API}{\gls{application program interface}}

\newabbreviation[category={acronymcat}]{isr}{ISR}{interrupt service routine}

\newabbreviation[category={acronymcat}]{soc}{SoC}{system-on-a-chip}

\newabbreviation[category={acronymcat}]{elf}{ELF}{executable and linkable format}

\newabbreviation[category={acronymcat}]{ram}{RAM}{random access memory}

\newabbreviation[category={acronymcat}]{cvd}{CVD}{cardiovascular diseases}

\newabbreviation[category={acronymcat}]{lna}{LNA}{low noise amplifier}

\newabbreviation[category={acronymcat}]{vga}{VGA}{variable gain amplifier}

\newabbreviation[category={acronymcat}]{fpga}{FPGA}{field-programmable gate array}

\newabbreviation[category={acronymcat}]{asic}{ASIC}{application-specific integrated circuit}

\newabbreviation[category={acronymcat}]{fet}{FET}{field-effect transistor}

\newabbreviation[category={acronymcat}]{lsb}{LSB}{least significant bit}

\newabbreviation[category={acronymcat}]{vna}{VNA}{vector network analyzer}

\newabbreviation[category={acronymcat}]{ml}{ML}{machine learning}

\newabbreviation[category={acronymcat}]{daq}{DAQ}{data acquisition}

\newabbreviation[category={acronymcat}]{afg}{AFG}{arbitrary function generator}

\newabbreviation[category={acronymcat}]{hal}{HAL}{hardware abstraction layer}

\newabbreviation[category={acronymcat}]{dcps}{DCPS}{\gls{dc} power supply}

\newabbreviation[category={acronymcat}]{gpio}{GPIO}{general purpose \gls{io}}

\newabbreviation[category={acronymcat}]{fsm}{FSM}{finite state machine}

\newabbreviation[category={acronymcat}]{vhsic}{VHSIC}{very high speed integrated circuits}

\newabbreviation[category={acronymcat}]{vhdl}{VHDL}{\gls{vhsic} hardware description language}

\newabbreviation[category={acronymcat}]{ide}{IDE}{integrated development environment}

\newabbreviation[category={acronymcat}]{oe}{OE}{operation enable}

\newabbreviation[category={acronymcat}]{dft}{DFT}{discrete Fourier transform}
